...
this.detailfact.myschedule(this.ls.attendeeId).success((res)=>{
  ...
  if(res.code==3||res.code==4){
    ...
    this.scope.errormsg="";
       var dummyDay,tempMonth,mon,cnt=0;
       this.indexArray=[];
       this.newArray=[];
       this.newRes={};
...
// console.log(String(String(res[1].startsOn.date).split(' ')[0]));
      for(let p=1,j=0;p<res.length;p++){
        ...
        this.newArray.push(this.newRes);
        this.newRes={};
        if(dummyDate>=todayDate && cnt==0){
       this.scope.currentTab=this.indexArray.indexOf(String(moment(dummyDate).format().split('T')[0]));
       console.log("dummyDate Myschedule if",String(moment(dummyDate).format().split('T')[0]),String(moment(todayDate).format().split('T')[0]),i);
       cnt++;
     }
     else{
       dummyDate=Date.parse(String(moment(res[p].startsOn.date).format().split('T')[0]));
       console.log("dummyDate Myschedule else",String(moment(res[p].startsOn.date).format().split('T')[0]),String(moment(todayDate).format().split('T')[0]),i);
